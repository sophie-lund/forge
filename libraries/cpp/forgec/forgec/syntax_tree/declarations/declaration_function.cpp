...
namespace forge {
const lt::NodeKind DeclarationFunction::NODE_KIND = NODE_DECLARATION_FUNCTION;
...
void DeclarationFunction::on_accept(lt::IVisitor& visitor) {
  visitor.visit(args);
...
bool DeclarationFunction::on_compare_declaration(
...
std::shared_ptr<lt::BaseNode> DeclarationFunction::on_clone() const {
  return std::make_shared<DeclarationFunction>(
      lt::SourceRange(source_range), std::string(name), clone_node_vector(args),
