...
TEST(language_syntax_tree_declarations_declaration_type_alias, node_auto) {
  std::stringstream debug_formatter_stream;
  lt::DebugFormatter debug_formatter(debug_formatter_stream);

  EXPECT_TRUE((gtest_node_auto_assert(NODE_DECLARATION_TYPE_ALIAS,
                                      debug_formatter, debug_formatter_stream,
                                      std::make_shared<DeclarationTypeAlias>(
                                          lt::SourceRange(), "x", nullptr))));
