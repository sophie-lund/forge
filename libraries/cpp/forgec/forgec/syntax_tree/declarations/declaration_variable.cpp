...
namespace forge {
const lt::NodeKind DeclarationVariable::NODE_KIND = NODE_DECLARATION_VARIABLE;
...
void DeclarationVariable::on_accept(lt::IVisitor& visitor) {
  visitor.visit(type);
...
bool DeclarationVariable::on_compare_declaration(
...
std::shared_ptr<lt::BaseNode> DeclarationVariable::on_clone() const {
  return std::make_shared<DeclarationVariable>(
      lt::SourceRange(source_range), std::string(name), clone_node(type),
