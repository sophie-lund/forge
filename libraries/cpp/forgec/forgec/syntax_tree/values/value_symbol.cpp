...
namespace forge {
const lt::NodeKind ValueSymbol::NODE_KIND = NODE_VALUE_SYMBOL;

ValueSymbol::ValueSymbol(lt::SourceRange&& source_range, std::string&& name)
    : BaseValue(NODE_KIND, std::move(source_range)), name(std::move(name)) {}

void ValueSymbol::on_accept(lt::IVisitor&) {}

void ValueSymbol::on_format_debug(lt::DebugFormatter& formatter) const {
  formatter.field_label("name");
  formatter.string(name);
}

std::shared_ptr<lt::BaseNode> ValueSymbol::on_clone() const {
  return std::make_shared<ValueSymbol>(lt::SourceRange(source_range),
                                       std::string(name));
}

bool ValueSymbol::on_compare(const lt::BaseNode& other) const {
...
void ValueSymbol::on_resolve_symbol(
