...
TEST(language_syntax_tree_values_value_binary, node_auto) {
  ...
  EXPECT_TRUE((gtest_node_auto_assert(
      ...
      std::make_shared<ValueBinary>(lt::SourceRange(), BinaryOperator::bool_and,
                                    nullptr, nullptr))));

  EXPECT_TRUE((gtest_node_auto_assert(
      NODE_VALUE_BINARY, debug_formatter, debug_formatter_stream,
      std::make_shared<ValueBinary>(
          lt::SourceRange(), BinaryOperator::bool_and,
