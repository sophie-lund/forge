...
TEST(language_syntax_tree_values_value_unary, node_auto) {
  ...
  EXPECT_TRUE((gtest_node_auto_assert(
      ...
      std::make_shared<ValueUnary>(lt::SourceRange(), UnaryOperator::bool_not,
                                   nullptr))));

  EXPECT_TRUE((gtest_node_auto_assert(
      NODE_VALUE_UNARY, debug_formatter, debug_formatter_stream,
      std::make_shared<ValueUnary>(
          lt::SourceRange(), UnaryOperator::bool_not,
