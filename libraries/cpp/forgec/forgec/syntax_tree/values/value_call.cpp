...
namespace forge {
const lt::NodeKind ValueCall::NODE_KIND = NODE_VALUE_CALL;

ValueCall::ValueCall(lt::SourceRange&& source_range,
...
void ValueCall::on_accept(lt::IVisitor& visitor) {
  visitor.visit(callee);
  visitor.visit(args);
}

void ValueCall::on_format_debug(lt::DebugFormatter& formatter) const {
...
std::shared_ptr<lt::BaseNode> ValueCall::on_clone() const {
  return std::make_shared<ValueCall>(lt::SourceRange(source_range),
                                     clone_node(callee),
                                     clone_node_vector(args));
}

bool ValueCall::on_compare(const lt::BaseNode& other) const {
