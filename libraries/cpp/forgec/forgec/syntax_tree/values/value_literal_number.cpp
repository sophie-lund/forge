...
namespace forge {
const lt::NodeKind ValueLiteralNumber::NODE_KIND = NODE_VALUE_LITERAL_NUMBER;

ValueLiteralNumber::ValueLiteralNumber(lt::SourceRange&& source_range,
...
void ValueLiteralNumber::on_accept(lt::IVisitor& visitor) {
  visitor.visit(type);
}

void ValueLiteralNumber::on_format_debug(lt::DebugFormatter& formatter) const {
  ...
  if (type) {
    if (type->type_with_bit_width_kind == TypeWithBitWidthKind::signed_int) {
      if (type->bit_width == 8) {
        ...
        LT_ABORT("unsupported bit width: " << type->bit_width);
      }
...
std::shared_ptr<lt::BaseNode> ValueLiteralNumber::on_clone() const {
  return std::make_shared<ValueLiteralNumber>(lt::SourceRange(source_range),
                                              clone_node(type), value);
}

bool ValueLiteralNumber::on_compare(const lt::BaseNode& other) const {
