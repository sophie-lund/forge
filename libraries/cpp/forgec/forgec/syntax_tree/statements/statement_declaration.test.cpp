...
TEST(language_syntax_tree_statements_statement_declaration, node_auto) {
  ...
  EXPECT_TRUE((gtest_node_auto_assert(
      NODE_STATEMENT_DECLARATION, debug_formatter, debug_formatter_stream,
      std::make_shared<StatementDeclaration>(lt::SourceRange(), nullptr))));

  EXPECT_TRUE((gtest_node_auto_assert(
      NODE_STATEMENT_DECLARATION, debug_formatter, debug_formatter_stream,
      std::make_shared<StatementDeclaration>(
          lt::SourceRange(), std::make_shared<DeclarationVariable>(
                                 lt::SourceRange(), "x", nullptr, nullptr)))));
}
