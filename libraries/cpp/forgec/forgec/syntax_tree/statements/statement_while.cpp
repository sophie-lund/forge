...
namespace forge {
const lt::NodeKind StatementWhile::NODE_KIND = NODE_STATEMENT_WHILE;

StatementWhile::StatementWhile(lt::SourceRange&& source_range,
...
void StatementWhile::on_accept(lt::IVisitor& visitor) {
  visitor.visit(condition);
  visitor.visit(body);
}

void StatementWhile::on_format_debug(lt::DebugFormatter& formatter) const {
...
std::shared_ptr<lt::BaseNode> StatementWhile::on_clone() const {
  auto clone = std::make_shared<StatementWhile>(
      lt::SourceRange(source_range), clone_node(condition), clone_node(body));
...
bool StatementWhile::on_compare(const lt::BaseNode& other) const {
