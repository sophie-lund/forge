...
namespace forge {
const lt::NodeKind TypeBasic::NODE_KIND = NODE_TYPE_BASIC;

TypeBasic::TypeBasic(lt::SourceRange&& source_range,
                     TypeBasicKind type_basic_kind)
    : BaseType(NODE_KIND, std::move(source_range)),
      type_basic_kind(type_basic_kind) {}

void TypeBasic::on_accept(lt::IVisitor&) {}

void TypeBasic::on_format_debug_type(lt::DebugFormatter& formatter) const {
...
std::shared_ptr<lt::BaseNode> TypeBasic::on_clone_type() const {
  return std::make_shared<TypeBasic>(lt::SourceRange(source_range),
                                     type_basic_kind);
}

bool TypeBasic::on_compare_type(const lt::BaseNode& other) const {
