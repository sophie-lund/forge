...
namespace forge {
const lt::NodeKind TypeFunction::NODE_KIND = NODE_TYPE_FUNCTION;

TypeFunction::TypeFunction(lt::SourceRange&& source_range,
...
void TypeFunction::on_accept(lt::IVisitor& visitor) {
  visitor.visit(return_type);
  visitor.visit(arg_types);
}

void TypeFunction::on_format_debug_type(lt::DebugFormatter& formatter) const {
...
std::shared_ptr<lt::BaseNode> TypeFunction::on_clone_type() const {
  return std::make_shared<TypeFunction>(lt::SourceRange(source_range),
                                        clone_node(return_type),
                                        clone_node_vector(arg_types));
}

bool TypeFunction::on_compare_type(const lt::BaseNode& other) const {
