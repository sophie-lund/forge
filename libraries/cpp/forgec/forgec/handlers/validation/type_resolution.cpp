...
void _trace_type_resolution(const TNodePointer& node_pointer) {
  if (node_pointer->resolved_type) {
    lt::trace("TypeResolutionHandler") << "resolved type to: ";
    format_type(FormattingOptions(lt::trace_stream()),
                node_pointer->resolved_type);
    lt::trace_stream() << std::endl;
  } else {
    lt::trace("TypeResolutionHandler")
...
lt::IHandler::Output TypeResolutionHandler::on_leave_value_literal_bool(
...
lt::IHandler::Output TypeResolutionHandler::on_leave_value_literal_number(
    Input<ValueLiteralNumber>& input) {
  LT_ASSERT(input.node()->type != nullptr,
...
lt::IHandler::Output TypeResolutionHandler::on_leave_value_symbol(
...
lt::IHandler::Output TypeResolutionHandler::on_leave_value_unary(
    Input<ValueUnary>& input) {
  LT_ASSERT(input.node()->operand != nullptr,
    ...
    case UnaryOperator::getaddr:
      if (input.node()->operand->resolved_type) {
        input.node()->resolved_type = std::make_shared<TypeUnary>(
            lt::SourceRange(), TypeUnaryKind::pointer,
            clone_node(input.node()->operand->resolved_type));
      }
...
lt::IHandler::Output TypeResolutionHandler::on_leave_value_binary(
    Input<ValueBinary>& input) {
  LT_ASSERT(input.node()->lhs != nullptr,
            "lhs is null - node is not well formed, was "
            "WellFormedValidationHandler run?");

  LT_ASSERT(input.node()->rhs != nullptr,
...
lt::IHandler::Output TypeResolutionHandler::on_leave_value_call(
    Input<ValueCall>& input) {
  LT_ASSERT(input.node()->callee != nullptr,
...
lt::IHandler::Output TypeResolutionHandler::on_leave_value_cast(
...
lt::IHandler::Output TypeResolutionHandler::on_leave_declaration_variable(
...
lt::IHandler::Output TypeResolutionHandler::on_leave_declaration_function(
  ...
  std::shared_ptr<TypeFunction> resolved_type = std::make_shared<TypeFunction>(
      lt::SourceRange(), clone_node(input.node()->return_type),
...
lt::IHandler::Output TypeResolutionHandler::on_leave_declaration_type_alias(
...
lt::IHandler::Output
TypeResolutionHandler::on_leave_declaration_structured_type(
    ...
    if (!members
             .emplace(member->name,
                      std::make_shared<DeclarationVariable>(
                          lt::SourceRange(), std::string(member->name),
