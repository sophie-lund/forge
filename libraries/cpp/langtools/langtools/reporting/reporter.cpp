...
#include <langtools/core/math_utilities.hpp>
#include <langtools/core/unicode.hpp>
#include <langtools/reporting/get_sample_lines.hpp>
...
void _print_message_source_range(std::ostream& stream,
  ...
  if (source_range.start.source != nullptr) {
    stream << termcolor::bright_blue << source_range.start
...
void _print_message_sample(std::ostream& stream,
  ...
  for (const SampleLine& line : sample_lines_result.lines) {
    ...
    LT_ASSERT(line_number_digits <= max_line_number_digits,
    ...
    stream << termcolor::bright_grey << line.line_number << "  "
           << termcolor::reset;
...
void _print_message(std::ostream& stream, const MessageContext& message_context,
  ...
  if (message.code.has_value()) {
    stream << termcolor::reset << termcolor::bright_grey << " "
           << message.code.value();
  }
...
void report_messages(std::ostream& stream,
  ...
  // Sort messages
  std::vector<lt::Message> messages_sorted = message_context.messages_sorted();
