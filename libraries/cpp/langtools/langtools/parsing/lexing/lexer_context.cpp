...
std::u16string_view LexerContext::read_next_grapheme_cluster() {
  LT_ASSERT(_current_location.source != nullptr, "source must not be null");
  LT_ASSERT(_current_location.line.has_value(), "line number must not be null");
  LT_ASSERT(_current_location.column.has_value(),
            "column number must not be null");
  LT_ASSERT(_current_location.offset.has_value(),
...
std::u16string_view LexerContext::current_value() const {
  LT_ASSERT(_current_location.source != nullptr, "source must not be null");
  LT_ASSERT(_token_start_location.offset.has_value(),
            "token start offset must not be null");
  LT_ASSERT(_token_start_location.offset.value() <=
                static_cast<size_t>(
  ...
  LT_ASSERT(_current_location.offset.value() <=
                static_cast<size_t>(
                    _current_location.source->content.value().length()),
