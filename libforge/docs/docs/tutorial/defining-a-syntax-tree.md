...
All of the code that is referenced here can be found in [`ExampleLanguage.cpp`](https://github.com/sophie-lund/grove/blob/main/projects/sycamore/v1/test/sycamore-syntaxtree/ExampleLanguage.cpp), although it's broken up here to be a lot easier to follow than one big C++ file.
...
`Node` is an abstract base class provided by Sycamore for all nodes in any programming language. All you need to do is provide your own constructor that takes in two arguments:
...
The child nodes are all wrapped with `std::shared_ptr<...>`. This type of smart pointer makes tree operations much simpler and more readable, so it's required by a lot of Sycamore code.
...
See the full example in [`ExampleLanguage.cpp`](https://github.com/sophie-lund/grove/blob/main/projects/sycamore/v1/test/sycamore-syntaxtree/ExampleLanguage.cpp) in the [syntax tree](https://github.com/sophie-lund/grove/blob/500ba5f38016e436810528c84af5623e6c9ea6e5/projects/sycamore/v1/test/sycamore-syntaxtree/ExampleLanguage.cpp#L28) section. This will implement every node type that we've discussed above as well as all of the ones defined in the [`ExampleNodeKind`](#node-kind-enum) enum.
