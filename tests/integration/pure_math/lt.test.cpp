...
TEST(integration_pure_math_lt, simple_u32_with_u32) {
  runIntegrationTest(
      {.source = "func f(a: u32, b: u32) -> bool {\n"
                 "  return a < b;\n"
                 "}\n",
       .on_jit_context = [](const JITContext& jit_context) {
         auto f =
             jit_context.try_lookup_function<bool (*)(uint32_t, uint32_t)>("f");

         ASSERT_EQ(f(0, 0), false);
         ASSERT_EQ(f(1, 0), false);
         ASSERT_EQ(f(0, 1), true);
         ASSERT_EQ(f(1, 1), false);
       }});
}

TEST(integration_pure_math_lt, simple_i32_with_i32) {
  runIntegrationTest(
...
TEST(integration_pure_math_lt, simple_f32_with_f32) {
  runIntegrationTest(
      {.source = "func f(a: f32, b: f32) -> bool {\n"
       ...
       .on_jit_context = [](const JITContext& jit_context) {
         ...
         ASSERT_EQ(f(0.0f, 0.0f), false);
         ASSERT_EQ(f(1.0f, 0.0f), false);
         ASSERT_EQ(f(0.0f, 1.0f), true);
